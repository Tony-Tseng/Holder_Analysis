...
def get_wallet_nft_table(interest_addr, days=100):
    ...
    table_df = pd.DataFrame(NFT_info_list)
    return table_df
...
def get_wallet_nft_plot(interest_addr, contract_addr):
    ...
    for response in transaction:
        if (response["to_address"] == interest_addr.lower()):
            ...
            time_history.append(response['block_timestamp'])
    ...
    if len(time_history) == 0:
        time_history.append(a.strftime("%m/%d/%y"))
        new_balance.append(current_num)
        time_history.append(today.strftime("%m/%d/%y"))
        new_balance.append(current_num)
